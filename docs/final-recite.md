...
<u>**敏捷方法的特征**（背）</u>：小周期迭代式持续交付、快速响应变更、价值交付、自动化、遵循敏捷宣言的价值观。

> **知识点出处**：`软件质量与管理.第二讲.pdf` 和 `8敏捷概述.pdf` —— 敏捷方法的特征与常见误解。已考 2014、2015A、2015B、2016 简答。
...
| **高级** | + AC + CV + CPI + BAC + EAC | 成本节约还是超支？预计完工成本？ |
...
<u>**需求开发过程**（背）</u>：需求获取（"诱导"方式）→ 需求汇总 → 需求验证 → 需求文档制作
