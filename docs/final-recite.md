...
**广义软件过程** = 技术 + 人员 + 狭义过程。同义词：软件开发方法、软件开发过程（如：净室 Cleanroom、极限编程 XP、SCRUM、Gate 方法）。
...
| | 生命周期模型 | 软件过程 |
|---|---|---|
| **定义** | 对软件过程的一种人为划分 | 为了实现一个或多个事先定义目标而建立的一组实践的集合 |
...
- 大型软件系统的开发过程也是一个逐步学习和交流的过程，软件系统的交付不是一次完成，而是通过多个迭代周期逐步完成交付。
...
瀑布模型**不是单一模型**，而是一系列从简单到复杂的模型。

- 项目挑战越多，所需的过程元素越复杂。
- 常见失败原因：团队低估了项目挑战，选择了过于简单的瀑布模型。
...
- **软件危机**：落后的软件生产方式无法满足迅速增长的软件需求。1960 年代出现，NATO 1968 会议正式提出「软件工程」。
...
| 阶段 | 时间 | 软件应用特征 | 典型方法 |
|---|---|---|---|
| **软硬件一体化** | 50 年代~70 年代 | 软件依附硬件；功能单一；复杂度有限；几乎不需要需求变更；硬件太贵 | "Measure twice, cut once"（三思而后行）、Code and Fix |
| **软件成为独立产品** | 70 年代~90 年代 | 摆脱硬件束缚（OS）；功能强大；规模和复杂度剧增；个人电脑出现→普通人成为软件用户→需求多变、兼容性要求；来自市场的压力 | 形式化方法、结构化程序设计、瀑布模型、成熟度运动 |
| **网络化和服务化** | 90 年代中期迄今 | 功能更复杂，规模更大；用户数量急剧增加；快速演化和需求不确定；分发方式变化（SaaS） | 迭代式开发、敏捷方法、开源开发、DevOps |
...
- 要求工程师必须全身心参与工作，主观意愿上努力追求卓越
- 知识工作者是智力劳动者
- 管理者无法像管理工厂流水线那样管理知识工作者
...
**故事点（Story Point）**：Scrum 中用于估算 PBI 相对大小的单位。
...
```
客户需求 → 定义需求 → 概要设计 → 规模估算 → 资源估算 → 日程计划 → 开发产品 → 收集数据 → 过程分析
```
...
- 燃尽图（Burndown Chart）是 EVM 在敏捷项目中的简化变形
...
**Quality Journey（质量路径）顺序**：

```
稳定测试 → 提升进入测试前的产物质量 → 度量并稳定团队评审
→ 建立质量意识 → 度量并稳定个人评审 → 诉诸设计
→ 设计验证 → 缺陷预防 → 走向用户质量观
```
...
| 模板 | 全称 | 描述对象 | 覆盖维度 | 记忆点 |
|---|---|---|---|---|
| **OST** | Operational Specification Template | 操作场景、外部交互、正常/异常使用 | 外部 + 动态 | **外部行为** |
| **FST** | Functional Specification Template | 对外接口、类/继承关系、外部可见属性和方法 | 外部 + 静态 | **接口静态信息** |
| **SST** | State Specification Template | 程序所有状态、状态转移条件、转移动作 | 内部 + 动态 | **内部动态信息** |
| **LST** | Logical Specification Template | 内部静态逻辑、伪代码、形式化逻辑 | 内部 + 静态 | **内部静态/逻辑信息** |
...
- **五项价值观**：沟通、简单、反馈、勇气、尊重（第 2 版 2004 新增）
- **四项基本活动**：**编码、测试、倾听、设计**
- XP 三层体系：价值观 → 原则 → 实践
...
| 策略 | 特点 |
|---|---|
| **大爆炸集成** | 所有组件一次性集成，适合质量普遍较高的组件 |
| **逐一添加集成** | 逐个添加组件集成，CI/CD 本质上就是此策略 |
| **集簇集成** | 按功能簇分组集成，有助于复用策略实现，可尽早获取可工作组件 |
| **扁平化集成** | 快速暴露系统级别错误，但对系统级错误的暴露能力有限 |

⚠️ 当待集成组件质量普遍不高时，不可使用扁平化策略。扁平化策略和集簇式策略的优缺点正好相反。

**集成策略选择考虑因素**：

- 待集成组件的**质量状态**（质量低→慎用扁平化，质量高→可大爆炸）
- 待集成组件的**获取方式**
- 待集成组件的**功能和关系**
- 待集成组件的**数量**
...
- **配置项**：受配置管理控制的工作产品（接口设计文档、源代码、用户手册、培训材料等）
- **基线**：经过正式评审和批准的配置项集合，作为后续工作的基础
...
**规模（Size）、时间（Time）、缺陷（Defect）**。⚠️ "风险"不是 PSP 基本度量元。
...
21. **质量三要素挑战**：目标、状态、纠偏
22. **面向用户质量观**：答题四问
23. **A/FR** = 质检成本/失效成本，期望值 ≈ 2.0
24. **PQI** 五个因子，越接近 1 越好
25. **Yield**：Phase Yield / Process Yield，可用于预测
26. **DRL**：以单元测试每小时发现缺陷率为基准
27. **Quality Journey 顺序**：稳定测试 → … → 走向用户质量观
28. **PSP 四大设计模板**：OST（外部行为）/ FST（接口）/ SST（内部动态）/ LST（内部静态）
29. **设计三层**：系统层（OST）→ 组件层（FST）→ 模块层（SST+LST）
30. **DevOps Three Ways**：从左到右流动 / 从右到左反馈 / 持续学习
31. **CI/CD/持续部署** 三层递进（不是同义词！）
32. **构建流水线**：提交 ≤10min → 集成 ~30min → E2E ~60-90min → 性能/安全
33. **结对编程经济账**：+15% 时间，-15% 缺陷
34. **TDD 实证**：-40~50% 缺陷密度
...
42. **PSP 三个基本度量元**：规模、时间、缺陷（风险不是！）
43. **评审 vs 测试**：IBM 1x vs 4.1x；JPL $90 vs $10,000
44. **质量定义**：DeMarco（用户满意度）、Weinberg（对人的价值）
45. **集成策略四种**：大爆炸/逐一添加/集簇/扁平化（CI/CD 是逐一添加）
46. **V&V 区别**：验证（与需求一致）/ 确认（目标环境正确）
47. **决策分析过程**（5 步）+ **根因分析四角度**：技术/人员/培训/过程
48. **PMBOK 九大知识领域**
49. **UML vs PSP 模板**：用例图→OST；状态图≈SST；LST 无 UML 对应
