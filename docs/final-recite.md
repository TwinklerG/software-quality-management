...
- <u>燃尽图（Burndown Chart）是 EVM 在敏捷项目中的简化变形</u>
...
验证是验证**答案**是否正确做出来；确认是确认**问题**是否被解决
...
- <u>**配置项**</u>：受配置管理控制的工作产品（接口设计文档、源代码、用户手册、培训材料等）
- <u>**基线**</u>：经过正式评审和批准的配置项集合，作为后续工作的基础
...
30. **DevOps Three Ways**：从左到右流动 / 从右到左反馈 / 持续学习；优势：敏捷管理、CI/CD优化、协作、自动化（2018）；不足：文化变革难、工具链复杂、学习曲线陡、安全风险
...
48. **PMBOK 九大知识领域**
49. **UML vs PSP 模板**：用例图/时序图→OST；类图不描述行为；FST有行为描述≠类图；LST无UML对应；SST状态转换信息超UML状态图（Slide 38）
