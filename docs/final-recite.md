...
| 阶段 | 时间 | 软件应用特征 | 典型方法 |
|---|---|---|---|
| **软硬件一体化** | 50 年代~70 年代 | 软件依附硬件；功能单一；复杂度有限；几乎不需要需求变更；硬件太贵 | "Measure twice, cut once"（代码评审）、Code and Fix、线性顺序过程（硬件开发） |
...
```
概要设计 → 代理识别和代理规模(E) → 估算并调整资源 → 计算预测区间
```
